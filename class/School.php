...
class School extends Dbconfig
{
	...
	public function adminLogin()
	{
		...
		if (!empty($_POST["login"]) && $_POST["email"] != '' && $_POST["password"] != '') {
			...
			$resultSet = mysqli_query($this->dbConnect, $sqlQuery) or die("error" . mysqli_error($this->dbConnect));
	...
	public function listStudent()
	{
		...
		if ($_POST["length"] != -1) {
			$sqlQuery .= 'LIMIT ' . (int) $_POST['start'] . ', ' . (int) $_POST['length'];
		}
	...
	public function addTeacher()
	{
		if ($_POST["teacher_name"]) {
			$insertQuery = "INSERT INTO " . $this->teacherTable . "(teacher, subject_id) 
				VALUES ('" . $_POST["teacher_name"] . "', " . $_POST["subject_id"] . ")";
			$userSaved = mysqli_query($this->dbConnect, $insertQuery);
		}
	}
	...
	public function updateTeacher()
	{
		if ($_POST['teacherid']) {
			...
			SET teacher = '" . $_POST["teacher_name"] . "', subject_id = " . $_POST["subject_id"] . "
			WHERE teacher_id ='" . $_POST["teacherid"] . "'";
			$isUpdated = mysqli_query($this->dbConnect, $updateQuery);
		}
	}
